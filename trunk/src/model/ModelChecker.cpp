...
double ModelChecker::calcProb(IntervalSet* iSet, double(*cdf)(double, FunctionVars*), double shift, FunctionVars* fv){
	double prob = 0;
	for (unsigned int i = 0; i < iSet->intervals.size(); i++)
        prob += ( cdf(iSet->intervals[i].end - shift, fv) - cdf(iSet->intervals[i].start - shift, fv));
	return prob;
